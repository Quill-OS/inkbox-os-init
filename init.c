...
int main(void) {
    ...
    if (MATCH(device, "emu")) {
        strcpy(tty, "ttyAMA0");
    } else if (MATCH(device, "bpi") || MATCH(device, "n367") ||
               MATCH(device, "n428")) {
        strcpy(tty, "ttyS0");
    } else {
        strcpy(tty, "ttymxc0");
    ...
    if (MATCH(device, "n249")) {
        ...
        REAP("/usr/bin/unsquashfs", "-d", "/lib/firmware/imx/epdc",
             "/opt/firmware.sqsh");

        // Load some modules
        load_module("/lib/modules/5.16.0-inkbox/kernel/drivers/power/supply/"
                    "rn5t618_power.ko",
                    "");
        load_module(
            "/lib/modules/5.16.0-inkbox/kernel/drivers/hwmon/tps6518x-hwmon.ko",
            "");
        load_module("/lib/modules/5.16.0-inkbox/kernel/drivers/regulator/"
                    "tps6518x-regulator.ko",
                    "");
        load_module("/lib/modules/5.16.0-inkbox/kernel/drivers/video/fbdev/mxc/"
                    "mxc_epdc_v2_fb.ko",
                    "");
        load_module("/lib/modules/5.16.0-inkbox/kernel/drivers/video/backlight/"
                    "lm3630a_bl.ko",
                    "");
        load_module(
            "/lib/modules/5.16.0-inkbox/kernel/drivers/rtc/rtc-rc5t619.ko", "");
        ...
        // If something doesn't work, go look up the issue and don't create code
        // that works but is hacky so it soon won't work IPD should just use
        // by-path and not directly that

        // Initial screen refresh
        // Leaving this for later; strangely, this causes the Clara HD's display
        // to shift vertically; go find out why
    ...
    if (MATCH(device, "kt")) {
        ...
        // eink_fb_waveform
        load_module(
            "/lib/modules/2.6.35-inkbox/kernel/drivers/video/eink/waveform/"
            "eink_fb_waveform.ko",
            "");
        ...
        if (FILE_EXISTS("/mnt/waveform/waveform.wbf") &&
            FILE_EXISTS("/mnt/waveform/waveform.wrf")) {
            // Use device waveform
            load_module("/lib/modules/2.6.35-inkbox/kernel/drivers/video/mxc/"
                        "mxc_epdc_fb.ko",
                        "default_panel_hw_init=1 default_update_mode=1 "
                        "waveform_to_use=/mnt/waveform/waveform.wbf");
        } else {
            // Use built-in waveform
            load_module("/lib/modules/2.6.35-inkbox/kernel/drivers/video/mxc/"
                        "mxc_epdc_fb.ko",
                        "default_panel_hw_init=1 default_update_mode=1 "
                        "waveform_to_use=built-in");
        }
        // Unmounting P1
        umount("/mnt");
    ...
    if (is_display_debug) {
        ...
        pid_t splash_pid = RUN("/etc/init.d/inkbox-splash",
                               "display_debug"); // FIXME: Kill and collect?
        sleep(5);
    ...
    int size = snprintf(kernel_version, 0U, "%s %s %s", uname_data.sysname,
                        uname_data.nodename, uname_data.version);
    if (size < 0) {
        ...
        if (snprintf(kernel_version, (size_t)size + 1U, "%s %s %s",
                     uname_data.sysname, uname_data.nodename,
                     uname_data.version) < 0) {
            perror("snprintf");
    ...
    if (encrypt_lock) {
        ...
        if (current_epoch < lock_epoch) {
            info("Too many incorrect encrypted storage unlocking attempts have "
                 "locked down this device. Shutting down ...",
                 INFO_FATAL);
            // Splash time
    ...
    while (true) {
        ...
        if (poll_num > 0) {
            // Drain stdin first
            char buf[PIPE_BUF]; // stdin should be line-buffered if it's a
                                // terminal, that should be more than large
                                // enough
    ...
    if (NOT_MATCH(device, "kt")) {
        ...
        while (true) {
            /*
             * Linux modifies tv to reflect the amount of time not slept (i.e.,
             * the amount of time left in the timeout), we rely on that to only
             * loop for 3s max without having to compute anything ourselves.
             */
            ...
            if (rv == -1 && errno != EINTR) {
                ...
                if (read(fd, &ev, sizeof(ev)) == sizeof(ev)) {
                    ...
                    // KEY_KATAKANA represents the brightness button on the Glo
                    // (N613)
    ...
    if (root) {
        ...
        if ((power_button_pressed && other_button_pressed) ||
            MATCH(dfl, "true")) {
    ...
    if (boot_mode == BOOT_STANDARD) {
        ...
        if (!is_display_debug) {
            ...
            if (MATCH(will_update, "true")) {
                do_update = true;
                pid_t splash_pid =
                    RUN("/etc/init.d/inkbox-splash",
                        "update_splash"); // FIXME: Kill & collect?
            } else {
                ...
                pid_t splash_pid =
                    RUN("/etc/init.d/inkbox-splash",
                        "progress_bar_init"); // FIXME: Kill & collect?
                sleep(2);
        ...
        if (root) {
            if (MATCH(login_shell, "bash")) {
                REAP("/bin/sed", "-i", "1s#.*#root:x:0:0:root:/root:/bin/bash#",
                     "/opt/passwd_root");
                REAP("/bin/sed", "-i",
                ...
                REAP("/bin/sed", "-i",
                     ...
                     "/opt/passwd_root");
                REAP("/bin/sed", "-i",
                     "30s#.*#user:x:1000:1000:Linux "
                     "User,,,:/:/usr/local/bin/zsh#",
                     "/opt/passwd_root");
                ...
                write_file(
                    "/mnt/root/.config/fish/fish_variables",
                    "# This file contains fish universal variable "
                    "definitions.\n# "
                    "VERSION: 3.0\nSETUVAR __fish_init_2_3_0:\\x1d\nSETUVAR "
                    "__fish_init_3_x:\\x1d\nSETUVAR --export "
                    "fish_user_paths:/usr/local/bin");
            } else {
                if (login_shell != NULL) {
                    if (NOT_MATCH(login_shell, "") &&
                        NOT_MATCH(login_shell, "ash")) {
                        char message_buff[256] = {0};
                        snprintf(message_buff, sizeof(message_buff),
                                 "'%s' is not a valid login shell; falling "
                                 "back to default",
                                 login_shell);
        ...
        // passwd file
        // Bind-mounting directly from initrd filesystem does not seem to work;
        // copying file to temporary filesystem
        if (root) {
        ...
        write_file(
            ...
            "doesn't work, first enable read-write support, then begin a "
            "connection with sshfs\n");
        ...
        if (FILE_EXISTS("/opt/firmware.sqsh")) {
            ...
            MOUNT("/dev/loop4", "/mnt/lib/firmware", "squashfs",
        ...
        // DHCPcd
        MOUNT("tmpfs", "/mnt/var/db/dhcpcd", "tmpfs",
              MS_NODEV | MS_NOSUID | MS_NOEXEC, "size=512K");
        write_file("/mnt/var/db/dhcpcd/duid", "");
        write_file("/mnt/opt/storage/dhcpcd_duid", "");
        MOUNT("/mnt/opt/storage/dhcpcd_duid", "/mnt/var/db/dhcpcd/duid", "",
              MS_BIND, "");
        ...
        if (MATCH(developer_key, "true")) {
            pid_t splash_pid =
                RUN("/etc/init.d/inkbox-splash",
                    "developer_splash"); // FIXME: Kill & collect?
        }

        // GUI root filesystem
        // Validating digital signature
        long int rc = REAP("/usr/bin/openssl", "dgst", "-sha256", "-verify",
                           "/mnt/opt/key/public.pem", "-signature",
                           "/mnt/opt/storage/gui_rootfs.isa.dgst",
                           "/mnt/opt/storage/gui_rootfs.isa");
        if (rc != 0 && NOT_MATCH(developer_key, "true")) {
            info("GUI root filesystem's digital signature is invalid!",
                 INFO_FATAL);
            ...
            REAP("/bin/busybox", "chroot", "/mnt", "/bin/squashfuse",
                 "/opt/storage/gui_rootfs.isa", "/opt/gui_rootfs/read");
            // Setting up GUI root filesystem overlay
            REAP("/bin/busybox", "chroot", "/mnt", "/usr/local/bin/unionfs",
                 "-o", "cow,nonempty",
                 "/opt/gui_rootfs/write=RW:/opt/gui_rootfs/read=RO", "/kobo");
    ...
    if (wait) {
        ...
        if (ret != pid) {
            ...
            if (WIFEXITED(wstatus)) {
                ...
                // Help caller grok the difference between exited or terminated
                // by a signal by using a negative return value.
                return -(sigcode);
            }
            // If the child caught a SIGSTOP because of ptrace, we're not
            // handling it separately (i.e., we'll silently return `pid`). There
            // are facilities available to handle that, but they require greater
            // care in how the looping is handled (c.f., waitpid(2)).
        }
    }
...
char* read_file(const char* file_path, bool strip_newline) {
    ...
    if (fread(buffer, sizeof(*buffer), (size_t)bytes, fp) < (size_t)bytes ||
        ferror(fp) != 0) {
...
int set_if_up(const char* if_name) { return set_if_flags(if_name, IFF_UP); }
...
void launch_dfl(void) {
    ...
    if (MATCH(device, "n705") || MATCH(device, "n905b") ||
        MATCH(device, "n905c") || MATCH(device, "n613")) {
        load_module("/modules/arcotg_udc.ko", "");
    }
    if ((MATCH(device, "n306") && NOT_MATCH(device_variant, "n306c")) ||
        MATCH(device, "n249") || MATCH(device, "n873")) {
    ...
    if (NOT_MATCH(device, "emu")) {
        load_module("/modules/g_mass_storage.ko",
                    "file=/dev/mmcblk0 removable=y stall=0");
    }
    ...
    while (true) {
        info("This device is in DFL mode. Please reset it to resume normal "
             "operation.",
             INFO_OK);
        sleep(30);
    }
...
void setup_usb_debug(bool boot) {
    ...
    if (boot) {
        ...
        while (true) {
            info("This device is in boot-time USB debug mode. Please reset or "
                 "reboot "
...
void setup_usbnet(bool setup_dhcpd) {
    ...
    snprintf(options, options_size, "dev_addr=%s host_addr=%s",
             usbnet_device_address, usbnet_host_address);
    ...
    if (MATCH(device, "n705") || MATCH(device, "n905b") ||
        MATCH(device, "n905c") || MATCH(device, "n613") ||
        MATCH(device, "n236") || MATCH(device, "n437")) {
        load_module("/modules/g_ether.ko", options);
    } else if (MATCH(device, "n306") || MATCH(device, "n249") ||
               MATCH(device, "n873") || MATCH(device, "bpi")) {
        if (NOT_MATCH(device_variant, "n306c")) {
            ...
            if (FILE_EXISTS("/modules/drivers/usb/gadget/function/"
                            "usb_f_ecm_subset.ko")) {
                load_module(
        ...
        load_module(
            ...
            "");
        load_module(
            "/modules/2.6.35-inkbox/kernel/drivers/usb/gadget/g_ether.ko",
            options);
...
void read_sector(char* buffer, size_t len, const char* device_node,
                 off_t sector, size_t sector_size) {
...
void progress_sleep(void) { usleep(500); }

void kill_process(const char* name) {
    // If you think this is lazy work, then go there:
    // http://pkgs-inkbox.duckdns.org:25560/misc/kill_process_nonsense.txt
...
void mount_squashfs_archives(void) {
    ...
    if (root) {
        ...
        MOUNT("/dev/loop7", "/mnt/opt/root", "squashfs",
              MS_NODEV | MS_NOSUID | MS_NOEXEC, "");
    }
    // Public key
    REAP("/sbin/losetup", "/dev/loop6", "/opt/key.sqsh");
    MOUNT("/dev/loop6", "/mnt/opt/key", "squashfs",
          MS_NODEV | MS_NOSUID | MS_NOEXEC, "");
    // Modules
    REAP("/sbin/losetup", "/dev/loop5", "/opt/modules.sqsh");
    MOUNT("/dev/loop5", "/mnt/lib/modules", "squashfs",
          MS_NODEV | MS_NOSUID | MS_NOEXEC, "");
    // Make insmod and modprobe coexist with already existing shell scripts
    if (MATCH(device, "n249")) {
        MOUNT("/mnt/lib/modules/5.16.0-inkbox/kernel/", "/mnt/lib/modules", "",
              MS_BIND | MS_REC, "");
        MOUNT("/lib/modules/5.16.0-inkbox/", "/mnt/lib/modules/5.16.0-inkbox/",
              "", MS_BIND | MS_REC, "");
    }
}
